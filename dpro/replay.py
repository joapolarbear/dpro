...
class Device:
    def __init__(self, device_name, _replayer, 
            infi_para=False,
            comm_backend = "NCCL",
            init_device_time_in_ms=0,
            full_trace=False
        ):
    ...
    def exct(self, name, _last_end_time, step_idx):
        ...
        if duration == 0 and not self.full_trace:
...
class PeriodicDevice(Device):
    ...
    def __init__(self, *args, period=5, **kwargs):
        '''period: cycle time in ms'''
        super().__init__(*args, **kwargs)
...
class PSCommDevice(Device):
    ...
    def __init__(self, op_counter, device_name, *args, **kwargs):
...
class Replayer:
    ...
    def name2device(self, n):
        ...
        if cat == "Comm":
            ...
            if self.comm_backend == "default":
                if "SEND" in n:
                    device_id = gen_long_name(pid, cat, "SEND")
                elif "RECV" in n:
                    device_id = gen_long_name(pid, cat, "RECV")
                else:
                    raise ValueError(f"Invalid comm op name {n}")
            else:
                SingleLogger().warn("Should we consider SEND and RECV as one single thread?")
    ...
    def create_comm_kernel_device(self, device_name, infi_para=False):
        d = CommKernelDevice(device_name, self, 
                comm_backend = self.comm_backend,
                infi_para=infi_para,
                full_trace=self.full_trace)
        return d
    ...
    def create_periodic_device(self, device_name, infi_para=False):
        d = PeriodicDevice(device_name, self, 
                period=5, 
                comm_backend = self.comm_backend, 
                infi_para=infi_para, 
                full_trace=self.full_trace)
    ...
    def create_ps_comm_device(self, device_name, infi_para=False):
        d = PSCommDevice(self.op_counter, device_name, self,  
                comm_backend = self.comm_backend,
                infi_para=infi_para,
                full_trace=self.full_trace)
        return d
